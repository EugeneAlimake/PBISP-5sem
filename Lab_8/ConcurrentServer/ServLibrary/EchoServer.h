...
//Процессы EchoServer создаются процессом AcceptServer по одному для каждого успешного подключения  
//программы клиента. Основным назначением процесса EchoServer является прием данных по созданному  
//процессом AcceptServer подключению и отправка этих же данных без изменения обратно программе клиента.
//Условием окончания  работы сервера является получение от клиента пустого сегмента данных (имеющего нулевую длину).  
{
	DWORD rc = 0;
